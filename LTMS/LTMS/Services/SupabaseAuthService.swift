...
class SupabaseAuthService: ObservableObject {
    ...
    // MARK: - Admin User Creation (No OTP Required)
    
    /// Create a user (educator/learner) by admin without requiring email confirmation
    /// This is separate from the learner OTP signup flow
    func createUserByAdmin(
        ...
        guard SupabaseConfig.shared.isConfigured else {
            print("⚠️ Supabase not configured for admin user creation")
            throw SupabaseAuthError.unknown("Supabase not configured")
        }
        ...
        do {
            print("🔵 Admin creating user: \(email) with role: \(role.rawValue)")
            
            // Create auth user - Supabase will send confirmation email but we'll confirm manually
            let authResponse = try await client.auth.signUp(
                email: email,
                password: password
            )

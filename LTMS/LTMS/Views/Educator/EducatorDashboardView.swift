...
struct EducatorHomeView: View {
    ...
    private func loadCourses() async {
        ...
        do {
            // Load all courses from Supabase and filter by assigned educator
            ...
            courses = allCourses.filter { course in
                course.assignedEducatorId == userId
            }
            
            print("📚 Educator \(authService.currentUser?.fullName ?? "") has \(courses.count) assigned courses from Supabase")
        } catch {
            print("❌ Error loading educator courses: \(error)")
            courses = []
        }
    }
}
